...
if ( !class_exists( 'YITH_WCET_Email_Template_Helper_Premium' ) ) {
    ...
    class YITH_WCET_Email_Template_Helper_Premium extends YITH_WCET_Email_Template_Helper {
        ...
        public function print_pre_header( $email, $template ) {
            if ( apply_filters( 'yith_wcet_print_pre_header', true, $email, $template ) ) {
                ...
                if ( !is_null( $custom_pre_header ) ) {
                    ...
                    $pre_header = $this->yith_wcet_generate_pre_header_from_email( $email, $template );
                }
         ...
         * @return string
         */
        public function yith_wcet_generate_pre_header_from_email( $email, $template ) {
            $pre_header = '';
            if ( $email instanceof WC_Email ) {
                ...
                $pre_header = $email->heading . ' - ' . $content;
                $pre_header = substr( $pre_header, 0, 100 );
        ...
        public
        function premium_email_extra_settings( $settings, $templates_array ) {
            $settings[] = array(
